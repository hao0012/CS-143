...
<STR>\" {
          ...
          if (strErrorQueue.size() > 0) {
            int error = strErrorQueue[0];
            if (error == 1 || error == 4) {
...
<STR_ESCAPE>[nbft]  {
                      ...
                      else strErrorQueue.push_back(3);
}
<STR_ESCAPE>\0  {
                  strErrorQueue.push_back(4);
                  BEGIN(STR);
}
<STR_ESCAPE>[^\0]	{
                    if (i < MAX_STR_CONST - 1) {
...
<STR>\0		{ strErrorQueue.push_back(1); }
<STR>\n		{ strErrorQueue.push_back(2); }
<STR>.  {	
          if (i < MAX_STR_CONST - 1) {
            my_string[i++] = yytext[0];
          } 
